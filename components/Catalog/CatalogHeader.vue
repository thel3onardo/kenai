<template>
  <div class="flex flex-col w-full md:w-3/4 lg:w-2/4">
    <div class="py-4 flex flex-wrap select-none">
      <Nuxt-link
    ...
    <div class="flex py-3 select-none">
      <span
        class="material-icons text-yellow md-18 home__header__star-icon"
        v-for="star in vote_average_stars"
        :key="star"
        v-bind:num-labels="vote_average_stars"
        >start</span
      >
    ...
    <div class="py-4">
      <button class="bg-yellow text-dark px-12 py-2 rounded hover:opacity-70">
        Assistir agora
      </button>
    </div>
  </div>
...
export default {
  ...
  async created() {
    try {
      const genres_from_api = (
        await this.$axios.get(
          `https://api.themoviedb.org/3/genre/movie/list?api_key=732544339751a8291cc05e685efec0e9&language=en-US`
        )
      ).data.genres
